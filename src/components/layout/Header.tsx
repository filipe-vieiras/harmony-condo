...
export function Header({ onToggleMobileMenu, mobileMenuOpen }: HeaderProps) {
  ...
  // Fecha os menus ao tocar/clicar fora deles ou ao pressionar Esc.
  useEffect(() => {
            ...
            {showProfileMenu && (
              <div className="absolute right-0 mt-2 w-56 rounded-xl border border-slate-200 bg-white p-1.5 shadow-xl ring-1 ring-black/5">
                <div className="px-3 py-2 border-b border-slate-100 mb-1">
                  ...
                  <p className="text-[12px] text-slate-500">{currentUser?.email}</p>
                  {currentUser?.unidade && (
                    <span className={`mt-1 inline-block rounded-full border px-2 py-0.5 text-[12px] font-semibold ${roleLabels[userRole].badgeColor}`}>
